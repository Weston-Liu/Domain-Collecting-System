...
router.get('/csv', function (req, res) {
    ...
    Promise.all([res, getData()
            .then(generateFile, console.error)
            .then(updateViewTime, console.error)
        ])
        .then(sendFile, console.error)
    ...
    var getData = () => {
        return new Promise((resolve, reject) => {
            ...
            var sql = "SELECT * FROM `data` WHERE `viewTime` = `createTime`";
    ...
    var generateFile = (results) => {
        return new Promise((resolve, reject) => {
            console.log('generating file...');

            var date = new Date();
            var filename = `${date.getFullYear()}-${date.getMonth()}-${date.getDate()} ${date.getHours()}-${date.getMinutes()}-${date.getSeconds()}.csv`;
            var ret = 'Site-ID,domain,unitType\n';
            for (let i = 0; i < results.length; i++) {
                ret += `${results[i].site},${results[i].domain},b2cUnit\n`;
            }
            fs.writeFile(path.join(__dirname, 'csv', filename), ret, (err) => {
                err && reject(err);
                resolve(date);
            });
        });
    ...
    var updateViewTime = (date) => {
        return new Promise((resolve, reject) => {
            ...
            var sql = 'UPDATE `data` SET `viewTime` = ? WHERE `viewTime` = `createTime`';
            var params = [ds]
