<template>
    <el-collapse v-model="activeNames">
        ...
        <!--el-button size="mini" type="danger" icon="delete" @click.stop="handleCountryEditClick(country.id)"></el-button>
        <el-button size="mini" icon="edit" @click.stop="handleCountryEditClick(country.id)"></el-button-->
      </span>
            </template>
            <el-table :data="country.sites" :show-header="false">
                ...
                <el-table-column label="Operate">
                    <template scope="scope">
                        <div class="right">
                            <!-- new item buttom -->
                            <el-button v-if="scope.row.new" size="small" @click="$emit('refsite')">Cancel</el-button>
                            <el-button v-if="scope.row.new" size="small" type="info" @click="handleSiteAdd(country.id)" icon="edit">Confirm</el-button>
                            <!-- normal button -->
                            <el-popover v-if="scope.row.new === undefined" width="300" placement="top" v-model="siteEditVisible[scope.row.id]">
...
<script>
    export default {
        ...
        methods: {
            handleCountryEditClick: function (id) {
                console.log(id);
            },
            ...
            handleSiteEdit: function (index, row) {
                fetch('api/admin/site', {
                    ...
                    if (res.ok) {
                        this.siteEditVisible[row.id] = false;
                        this.$message.success('Site name updated successfully.');
                        row.name = this.input;
                    } else {
